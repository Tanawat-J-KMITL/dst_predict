...
def predict(rec, indx, size=64, pred=6):
    ...
    def inputs(i):
        timestamps = slice(
            rolling(rec, "timestamp", indx_abs - size - pred, size + pred),
            i,
            size,
        )
        return {
            "dst_nT": slice(
                rolling(rec, "dst_nT", indx_abs - size - pred, size + pred),
                i,
                size,
            ),
    ...
    return np.array([inputs(i) for i in range(pred)])

def training(rec, indx: int, size=64, pred=6):

    indx_abs = indx + size + pred

    _check_bounds("training", rec, indx, size, pred)
...
def build_training_dataset(rec, size=64, pred=6):

    all_inputs = []
    all_truths = []

    max_indx = len(rec["data"]) - (size + pred) - 1

    for i in range(max_indx):
        ts = training(rec, i, size=size, pred=pred)
        all_inputs.extend(ts["inputs"])
        all_truths.extend(ts["truths"])
