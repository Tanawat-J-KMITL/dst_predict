...
def _error_bounds(name, rec, indx, size, pred):
    if indx < 0:
...
def _get_bounds(rec, indx, size, pred):
    return rec["data"].size - (size + pred)

def get_bounds_predict(rec, indx=0, size=64, pred=6):
    return _get_bounds(rec, indx, size, pred)

def get_bounds_training(rec, indx=0, size=64, pred=6):
    return _get_bounds(rec, indx, size + pred, pred)

def predict(rec, indx, size=64, pred=6):
    _error_bounds("predict", rec, indx, size, pred)
...
def training(rec, indx: int, size=64, pred=6):
    _error_bounds("training", rec, indx, size, pred)
    indx_abs = indx + size + pred
    return {
